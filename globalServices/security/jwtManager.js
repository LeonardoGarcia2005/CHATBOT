...
export async function createAccessToken(paraAccesoPublico, usuario) {
  ...
  if (
    paraAccesoPublico === false &&
    (!usuario ||
      !usuario.nombre_usuario ||
      usuario.nombre_usuario.trim().length == 0 ||
      !usuario.id ||
      usuario.id < 1)
  ) {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      {
        expiresIn:
          Date.now() + configurationProvider.security.jwtExpirationTime * 1000,
      }
    )
    return token
  } catch (error) {
...
const getUser = async (token) => {
  ...
  try {
    if (token && token.trim().length > 0) {
      ...
      loggerGlobal.debug(
        'el secret leido es: ' + configurationProvider.security.jwtSecret
      )
  ...
  if (
    payload &&
    payload.nombre_usuario != configurationProvider.security.jwtPublicAccessUser
  ) {
    loggerGlobal.debug('Evaluando la expiración del token ... ')
    const ahoraMismo = Date.now()
    if (payload && ahoraMismo > payload.expiresIn) {
      loggerGlobal.debug(
        'Expiracion del token es: ' + (payload.expiresIn - ahoraMismo)
      )
    ...
    loggerGlobal.debug(
      'Es un acceso publico; no se verificara expiracion del token'
    )
...
const jwtProvider = async ({ req, res }) => {
  loggerGlobal.debug(
    'El request header es: ' + JSON.stringify(req.headers.canal)
  )

  loggerGlobal.debug('El operationName es: ' + req.body.operationName)

  if (req.headers.canal === 'whatsapp') {
    return {}
  }
  ...
  loggerGlobal.debug(
    'El objeto session es: ' + req.session.sessionVariables?.sessionId
  )
  loggerGlobal.debug(
    'El token de la session es: ' + req.session.sessionVariables?.tokenJWT
  )
  ...
  if (nombre_usuario && nombre_usuario !== userNameCliente) {
    loggerGlobal.error(
      'El usuario extraido del token es inválido, no se corresponde con el usuario autenticado en la sesión'
    )
